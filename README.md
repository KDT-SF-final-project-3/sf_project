...
| 한용찬   | 팀원, HW 담당   | **프로젝트 기획**, 로봇팔 시스템 담당, 발표 자료 수집 <br> - **HW**: 상하 제어 DC 모터, 집게팔 제어 DC 모터 아두이노 회로 구현, 모터 선정 구매 및 제어, 센서 제어 <br> - **SW**: 아두이노 기반 제어 기능 코딩, DB-기능서버 연동                                | [![GitHub](https://img.shields.io/badge/GitHub-Profile-black?logo=github)](https://github.com/gksdydcks) |
...
| 1-8       | 페이지 구조화            | 안내 페이지, 웹캠 연결 페이지 구성                       | 유승태 ,김희수           | Medium |
...
- 서버를 실행시키는 중 간헐적으로 서버가 멈춰 새로고침을 해야하는 현상이 발생.
- 각각의 서버간의 통신 중 딜레이에 따른 현상으로 추정.
- 추후 프로젝트에서 고려하여 서버 설계할 예정.


### HW 부분
#### 1. 미세한 각도 조절이 되지 않았음
- 스텝 모터의 기본 스펙이 200 steps/rev 라서 1 step당 1.8도를 제어할 수 있었음.
- 좀 더 정밀한 각도를 제어하기 위해 마이크로 스테핑 모드를 사용하였음.
    ⇒ 마이크로 스테핑 모드(1/16)을 사용하여 1 STEP당 0.1125도를 제어할 수 있었음.
    
#### 2. 마이크로 스테핑 모드를 사용하였으나 원하고자 하는 각도만큼 정확히 회전 불가
- 분석 1 : 모터의 속도가 빨라서 따라가지 못함.
    - 스텝수의 3% 남은 구간을 감속구간으로 설정하고, COS함수를 사용하여 부드럽게 정지를 하며 모터의 관성에 의한 오버슈트를 최소화 하였음.
    - 이를 통해 원하는 각도에 보다 정확히 도달할 수 있었고, 정지 시 진동도 줄어들어 전체적인 제어 안정성이 향상되었음.
    
- 분석 2 :  회전 스텝 수를 계산하여 보정 하자
    - 마이크로스테핑 모드에서는 이론적으로 정밀하지만, 드라이버 특성, 부하, 토크 부족 등으로 인해 실제 위치와 목표 위치 간에 오차가 누적.
    - 조치 내용: 회전 스텝 수를 기반으로 실제 위치를 계산하여 목표 위치와 비교 후 보정하는 방식을 적용하였음. 이를 통해 누적 오차를 줄이고, 반복적인 동작에서도 보다 정밀한 위치 제어가 가능하도록 하였음.
