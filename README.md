...
### HW 부분
#### 1. 미세한 각도 조절이 되지 않았습니다.
- 스텝 모터의 기본 스펙이 200 steps/rev 라서 1 step당 1.8도를 제어할 수 있었습니다.
- 좀 더 정밀한 각도를 제어하기 위해 마이크로 스테핑 모드를 사용하였습니다. 
    ⇒ 마이크로 스테핑 모드(1/16)을 사용하여 1 STEP당 0.1125도를 제어할 수 있었습니다.
    
#### 2. 마이크로 스테핑 모드를 사용하였으나 원하고자 하는 각도만큼 정확히 회전하지 않았습니다.
-  분석 1 : 모터의 속도가 빨라서 따라가지 못하는 것이다.
    ⇒ 스텝수의 3% 남은 구간을 감속구간으로 설정하고, COS함수를 사용하여 부드럽게 정지를 하며 모터의 관성에 의한 오버슈트를 최소화 하였습니다. 이를 통해 원하는 각도에 보다 정확히 도달할 수 있었고, 정지 시 진동도 줄어들어 전체적인 제어 안정성이 향상되었다
    
- 분석 2 :  회전 스텝 수를 계산하여 보정 하자
    ⇒ 마이크로스테핑 모드에서는 이론적으로 정밀하지만, 드라이버 특성, 부하, 토크 부족 등으로 인해 실제 위치와 목표 위치 간에 오차가 누적될 수 있습니다.
    ⇒ 조치 내용: 회전 스텝 수를 기반으로 실제 위치를 계산하여 목표 위치와 비교 후 보정하는 방식을 적용하였습니다. 이를 통해 누적 오차를 줄이고, 반복적인 동작에서도 보다 정밀한 위치 제어가 가능하도록 하였습니다.
